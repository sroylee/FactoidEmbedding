...
def embed(X, n_dim, n_iter):
    ...
    with graph.as_default():
        ...
        mt = tf.nn.embedding_lookup(attribute_embeddings, input_pairs_i) * tf.nn.embedding_lookup(attribute_embeddings, input_pairs_j)
        mt = tf.reduce_sum(mt, axis=1)
        loss = tf.nn.l2_loss(mt - input_xs, name='loss') * 2 / BATCH_SIZE

        learning_rate = tf.train.exponential_decay(LEARNING_RATE*np.sqrt(n_dim) / 2, global_step, 25*n_W/BATCH_SIZE, 0.96, staircase=True)
